  ...
  Future<void> fetchOrderHistory(String request) async {
    try {
      ...
      OrderHistory? orderHistoryResponse =
          await _mainRepository.postOrderHistory(schoolID: request);

      if (orderHistoryResponse != null &&
          orderHistoryResponse.data.orders?.isNotEmpty == true) {
        ...
        emit(OrderHistoryErrorState(
            orderHistoryResponse?.message ?? "No orders found."));
      }
      ...
      if (fieldErrors.isNotEmpty) {
        ...
        emit(OrderHistoryErrorState(
            "Unexpected error occurred while fetching order history."));
      }
